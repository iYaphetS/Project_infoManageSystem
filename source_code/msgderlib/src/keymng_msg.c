...
int MsgKey_Req_Encode(MsgKey_Req *pStruct, ITCAST_ANYBUF **outData)
{
	...
	if (ret != 0)
	{
		...
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func DER_ItAsn1_WriteInteger() err");
		return ret;
	}
	pTmp = pHead;
	...
	{
		DER_ITCAST_FreeQueue(pHead);
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func EncodeChar() err");
		return ret;
	}
	pTmp = pTmp->next;
	...
	{
		DER_ITCAST_FreeQueue(pHead);
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func DER_ItAsn1_WriteSequence() err");
		return ret;
	}
	DER_ITCAST_FreeQueue(pHead);
...
int MsgKey_Req_Decode(ITCAST_ANYBUF *inData, MsgKey_Req **pStruct)
{
	...
	{
		ret = KeyMng_ParamErr;
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func MsgKey_Req_Decode() err check (inData==NULL || pStruct==NULL)");
		return ret;
	}
	...
	if (ret != 0)
	{
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func DER_ItAsn1_ReadSequence()");
		return ret;
	}
	pTmp = pHead;
	...
	if (pTmpStru == NULL)
	{
		...
		DER_ITCAST_FreeQueue(pHead);//Free Mem
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"check malloc err");
		return ret;
	}
	...
	if (ret != 0)
	{
		...
		DER_ITCAST_FreeQueue(pHead);
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func DER_ItAsn1_ReadInteger() err");
		return ret;
	}
	...
	if (ret != 0)
	{
		if (pTmpStru != NULL) free(pTmpStru);
		DER_ITCAST_FreeQueue(pHead);
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func DER_ItAsn1_ReadPrintableString() err");
		return ret;
	}
...
int MsgKey_Res_Decode(ITCAST_ANYBUF *inData, MsgKey_Res **pStruct)
{
	...
	{
		ret = KeyMng_ParamErr;
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func MsgKey_Res_Decode() err check  (inData==NULL || pStruct==NULL)");
		return ret;
	}
	...
	if (ret != 0)
	{
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func DER_ItAsn1_ReadSequence() err");
		return ret;
	}
	pTmp = pHead;
	...
	int				*outLen )
{
	...
	{
		ret = KeyMng_ParamErr;
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func MsgEncode() err  (pStruct == NULL || outData==NULL || outLen==NULL )");
	...
	case ID_MsgKey_Req:
		...
		{   
			DER_ITCAST_FreeQueue(pHeadbuf);
			ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func MsgKey_Req_Encode() err ");
			return ret;           
		}
		break;
	...
	case ID_MsgKey_Res:
		...
		{			 
			DER_ITCAST_FreeQueue(pHeadbuf);
			ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func MsgKey_Res_Encode() err ");
			return ret;		 
		}
		break;
	...
	int           *type /*out*/)
{
	...
	{
		ret = KeyMng_MallocErr;
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func MsgDecode() err, check malloc err ");
		return ret;
	}
	...
	*type = temp;
	if (pHeadBuf->next == NULL) //判断der格式是否正确
	{
		ret = KeyMng_ParamErr;
		DER_ITCAST_FreeQueue(pHeadBuf);
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"check  (pHeadBuf->next == NULL)");
		return ret;
	}	  
	...
	{   
		ret = KeyMng_ParamErr;
		DER_ITCAST_FreeQueue(pHeadBuf);
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"check  (判断der长度是否正确)");
		return ret;
	}	
	...
	case ID_MsgKey_Req:
		...
		{
			DER_ITCAST_FreeQueue(pHeadBuf);
			ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func MsgKey_Req_Decode() err");
			return ret;
		}
		break;
	case ID_MsgKey_Res:
		...
		{
			DER_ITCAST_FreeQueue(pHeadBuf);
			ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func MsgKey_Res_Decode() err");
			return ret;			 
		}
		break;
	default: 
		ret = KeyMng_TypeErr;
		DER_ITCAST_FreeQueue(pHeadBuf);
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"check type err");
		return KeyMng_TypeErr;
	}
...
int MsgMemFree(void **point,int type)
{
	...
	default:
		ret = KeyMng_TypeErr;
		ITDER_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func MsgMemFree() check type err");
		return ret;
	}
	return ret;
}
